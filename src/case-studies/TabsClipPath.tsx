...
export default function TabsClipPath() {
  ...
  return (
    <div className="center-container">
      <div className="relative flex flex-col items-center w-fit py-16 px-0">
        ...
        <div
          aria-hidden
          className="absolute z-10 w-full overflow-hidden tabs-clip-path-container"
          ref={containerRef}
        >
          <ul className="relative flex w-full justify-center gap-2 bg-[#2090ff]">
            {TABS.map((tab) => (
              <li key={tab.name}>
                <button
                  data-tab={tab.name}
                  ...
                  className="text-white flex h-8 items-center gap-2 rounded-full p-4 "
                  tabIndex={-1}
                >
                  {tab.icon}
                  {tab.name}
